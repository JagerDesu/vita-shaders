...
int unimplemented_syscall(int a1, int a2, int a3, int a4, int a5, int a6, int a7, int a8, int a9, int a10) { // will probably crash and burn with more than 10 args
	int lr;
	asm("mov %0,lr\n" : "=r" (lr));
	printf("Unimplemented, called from 0x%08x\n", lr);
	return -1;
}
...
void register_syscalls(void) {
	int count = 0;
#define REGISTER(nid_, func_) syscalls[count].nid = nid_; syscalls[count].func = func_; syscalls[count].name = #func_; ++count;
